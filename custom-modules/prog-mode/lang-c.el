...
(declare-function my-outline-mode/outline-level "system-tools")
...
(use-package cc-mode                                                              ; built-in
  ...
  (defun my/c-mark-block ()
    ...
    (let ((start (progn
      ...
      (push-mark end)
      (setq mark-active t)))
  (defun lsp-maybe-active-if-compile-commands-in-project-root ()
    "Enable `lsp' if the project root contains `compile_commands.json'.
  The project root is found using `project-root'."
    (let ((project-root-path (project-root)))
      (if (not project-root-path)
          t
        (file-exists-p
         (expand-file-name "compile_commands.json" project-root-path)))))
  (defun +c-mode-setup ()
    (lsp-maybe)
    (setq-local indent-tabs-mode nil)
    (setq-local show-trailing-whitespace t)
    (modify-syntax-entry ?_ "w"))
  (lsp-maybe-add-predicate 'c-mode #'lsp-maybe-active-if-compile-commands-in-project-root)
  :general
  (mode-action-leader
   'c-mode-base-map
   "." #'c-set-style
   ";" #'comment-dwim
   "c" #'project-compile-project
...
(use-package c-ts-mode                                                            ; built-in
  ...
  (defun +c-ts-mode-setup ()
    ...
    (setq-local show-trailing-whitespace t)
    (modify-syntax-entry ?_ "w"))
  (lsp-maybe-add-predicate 'c-ts-mode #'lsp-maybe-active-if-compile-commands-in-project-root)
  :hook
...
(defun my-lang/c-mode-setup ()
  ...
  (message
   "[%s ; DEBUG; my-lang-python/c-mode-setup]finished loading the defun ; ;"
   (current-time-string)))
