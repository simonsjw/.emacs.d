;;; llm-support.el --- gptel, aidermacs, and local/remote LLM backends. -*- lexical-binding: t; -*-
...
;; gptel, Aidermacs, and local/remote LLM backends (xAI Grok, Ollama).
;; Keys live in `keymaps-llm.el' (`C-c m').  Load from `init.el' after
;; `logging-config'.  Aidermacs expects the Aider CLI on `PATH'.
;;
;; Map:
;;   Feature:    llm-support
...
(defun my-llm/aidermacs-menu ()
  "Open the Aidermacs transient menu (C-c m a)."
  (interactive)
...
(defun my-llm/init ()
  ...
  (log/info :fn 'my-llm/init
            :msg "llm-support fully initialised. C-c m n (chat) • C-c m a (Aidermacs)"
            :obj t))
