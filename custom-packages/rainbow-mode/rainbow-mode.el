...
(defvar rainbow-info-theme-colors-font-lock-keywords nil
  "Font-lock keywords to add for info-theme colours.")
(make-variable-buffer-local 'rainbow-info-theme-colors-font-lock-keywords)
...
;;rainbow-info-theme-colors-alist

(defcustom rainbow-info-theme-colors-alist
  '(("dummy" . "#F0F8FF"))
  "Alist of info-theme colors.  Should be set externally. 
Each entry should have the form (COLOR-NAME . HEXADECIMAL-COLOR)."
  :type 'alist
  :group 'rainbow)
...
(defcustom rainbow-info-theme-colors 'auto
  "When to enable info-theme colours.

If set to t, the info-theme colors will be enabled.  If set to nil, the
info-theme colors will not be enabled.  If set to auto, the info-theme colors
will be enabled if a major mode has been detected from the
`rainbow-info-theme-colors-major-mode-list'."
  :type '(choice (symbol :tag "enable in certain modes" auto)
                 (symbol :tag "enable globally" t)
                 (symbol :tag "disable" nil))
  :group 'rainbow)
...
(defcustom rainbow-info-theme-colors-major-mode-list
  '(emacs-lisp-mode)
  "List of major mode where info-theme colors are enabled.

This is only active when `rainbow-info-theme-colors' is set to auto."
  :type '(repeat (symbol :tag "Major-Mode"))
  :group 'rainbow)
...
(defun rainbow-turn-on ()
  ...
  (when (or (eq rainbow-r-colors t)
    ...
    (font-lock-add-keywords nil
                            ...
                            t))
;; Activate info-theme colours?
  (when (or (eq rainbow-info-theme-colors t)
            (and (eq rainbow-info-theme-colors 'auto)
                 (memq major-mode rainbow-info-theme-colors-major-mode-list)))
    (setq rainbow-info-theme-colors-font-lock-keywords
          `((,(regexp-opt (mapcar #'car rainbow-info-theme-colors-alist) 'words)
             (0 (rainbow-colorize-by-assoc rainbow-info-theme-colors-alist)))))
    (font-lock-add-keywords nil
                            rainbow-info-theme-colors-font-lock-keywords
                            t)))
